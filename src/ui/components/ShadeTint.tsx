...
const ShadeTint = forwardRef<HTMLDivElement, ShadeTintProps>(
    ({className, style}, ref) => {
        ...
        const customTokenTint = useSwatchStore(state => state.getTint().customToken);
        const customTokenShade = useSwatchStore(state => state.getShade().customToken);
                ...
                <div className={"row"}>
                    <div className={"col col-6"}>
                        ...
                        <Swatch color={shadeColor} name={shadeName}
                                updateSwatch={function (color: string, name: string): void {
                                    setShade(color, name, name, customTokenShade);
                                    buildSwatches();
                                }}
                                id={shade.id}
                        />
                    ...
                    <div className={"col col-6"}>
                        ...
                        <Swatch color={tintColor} name={tintName}
                                updateSwatch={function (color: string, name: string): void {
                                    setTint(color, name, name, customTokenTint);
                                    buildSwatches();
                                }}
                                id={tint.id}
                        />
