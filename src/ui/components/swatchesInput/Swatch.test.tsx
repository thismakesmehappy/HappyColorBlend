...
describe('Swatch Component', () => {
  ...
  test('renders in display mode without controls', () => {
    render(<Swatch {...defaultProps} display={true} />);
    
    // The edit button should not be present in display mode
    expect(screen.queryByTestId('edit-button')).not.toBeInTheDocument();
  });

  test('renders in horizontal layout when specified', () => {
    render(<Swatch {...defaultProps} horizontal={true} />);
    
    // Check if the container has the hstack class
    const container = screen.getByTestId('swatch-container');
    expect(container).toHaveClass('hstack');
  });
  ...
  test('calls updateSwatch when component mounts', async () => {
    render(<Swatch {...defaultProps} />);
    
    // Wait for the useEffect to run
    await waitFor(() => {
      expect(mockUpdateSwatch).toHaveBeenCalledWith('FF5733', 'Test Color', '123');
    });
  });
